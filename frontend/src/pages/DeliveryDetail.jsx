...
import { ArrowLeft, ChevronRight, AlertCircle, Plus } from "lucide-react";
...
export default function DeliveryDetail() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [pickRes, locRes, stockRes, prodRes] = await Promise.all([
      ...
      console.error("DeliveryDetail fetch error:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!picking) return <div className="p-8" style={{ color: 'var(--odoo-purple)' }}>Delivery not found.</div>;

  const getLocationName = (locId) => locations.find(l => l.id === locId)?.name || "Unknown";
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  const toContact = getLocationName(picking.location_dest_id);
  ...
  const getStockWarning = (move) => {
    const quant = stock.find(s => s.product_id === move.product_id && s.location_id === picking.location_id);
    const available = quant ? quant.quantity : 0;
    return available < move.quantity;
  };
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-2 gap-16 mb-12">
            <div className="space-y-6">
              <div>
                <label className="text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--odoo-text-muted)' }}>Delivery Address</label>
                <div className="mt-1 border-b py-2 font-semibold text-lg" style={{ borderColor: 'var(--odoo-border)', color: 'var(--odoo-text)' }}>{toContact}</div>
              </div>
              <div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--odoo-text-muted)' }}>Operation Type</label>
                <div className="mt-1 border-b py-2 font-semibold text-lg flex justify-between items-center" style={{ borderColor: 'var(--odoo-border)', color: 'var(--odoo-text)' }}>
                  <span>{picking.type}</span>
                  <svg width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 1.5L6 6.5L11 1.5" stroke="var(--odoo-purple)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {picking.moves && picking.moves.map((move) => {
                  const isOutOfStock = getStockWarning(move) && !isDone;
                  return (
                    <tr key={move.id} className={`border-b hover:bg-gray-50 transition-colors text-sm ${isOutOfStock ? 'bg-red-50' : ''}`} style={{ borderColor: isOutOfStock ? '#DC354580' : 'var(--odoo-border)', color: isOutOfStock ? '#DC3545' : 'var(--odoo-text)' }}>
                      <td className="px-4 py-3 font-medium flex items-center gap-2">
                        {move.product ? `[${move.product.sku}] ${move.product.name}` : `Product #${move.product_id}`}
                        {isOutOfStock && <AlertCircle className="w-4 h-4 text-red-500" title="Product not in stock!" />}
                      </td>
