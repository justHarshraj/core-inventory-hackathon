...
export default function InternalDetail() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("InternalDetail fetch error:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!picking) return <div className="p-8" style={{ color: 'var(--odoo-purple)' }}>Internal Transfer not found.</div>;

  const getLocationName = (locId) => locations.find(l => l.id === locId)?.name || "Unknown";
  const user = JSON.parse(localStorage.getItem("user") || "{}");
  const sourceLocation = getLocationName(picking.location_id);
  const destLocation = getLocationName(picking.location_dest_id);
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-2 gap-16 mb-12">
            <div className="space-y-6">
              <div>
                <label className="text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--odoo-text-muted)' }}>Source Location</label>
                <div className="mt-1 border-b py-2 font-semibold text-lg" style={{ borderColor: 'var(--odoo-border)', color: 'var(--odoo-text)' }}>{sourceLocation}</div>
              </div>
              <div>
                <label className="text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--odoo-text-muted)' }}>Destination Location</label>
                <div className="mt-1 border-b py-2 font-semibold text-lg" style={{ borderColor: 'var(--odoo-border)', color: 'var(--odoo-text)' }}>{destLocation}</div>
              </div>
            </div>
            <div className="space-y-6">
              <div>
